...
def count_inversions(list1_images, list2_images_truths) :
  ...
  inversions  = sum(( 1 for i,j in itertools.combinations(range(len(order2)), 2) if order2[i][0]>order2[j][0] ))
  significant = sum(( 1 for i,j in itertools.combinations(range(len(order2)), 2) if order2[i][0]>order2[j][0] and order2[i][1]!=order2[j][1]))
