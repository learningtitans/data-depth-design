...
def main(unparsed):
  ...
  if not FLAGS.train_dir:
    raise ValueError('You must supply the training directory with --train_dir')
  ...
  with tf.Graph().as_default():
    ...
    with tf.device(deploy_config.inputs_device()):
      ...
      [image, label] = provider.get([dataset.input_field, FLAGS.task_name])
    ...
    def exit_gracefully(signum, frame):
      ...
      tf.logging.error('Interrupted on (UTC):', interrupted)
      return 1
    ...
    if not FLAGS.experiment_file is None:
      experiment_file = open(FLAGS.experiment_file, 'wt')
